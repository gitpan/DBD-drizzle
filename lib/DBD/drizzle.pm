...
sub driver{
    ...
    $drh = DBI::_new_drh($class, { 'Name' => 'drizzle',
				   'Version' => $VERSION,
				   'Err'    => \$DBD::drizzle::err,
...
sub connect {
    ...
    DBD::drizzle->_OdbcParse($dsn, $privateAttrHash,
				    ['database', 'host', 'port']);
...
sub admin {
	...
	shift : '';
    my($host, $port) = DBD::drizzle->_OdbcParseHost(shift(@_) || '');
    my($user) = shift || '';
    my($password) = shift || '';
...
%DBD::drizzle::db::ANSI2db = ("CHAR"          => "CHAR",
			   "VARCHAR"       => "CHAR",
			   "LONGVARCHAR"   => "CHAR",
			   "NUMERIC"       => "INTEGER",
			   "DECIMAL"       => "INTEGER",
...
sub prepare {
    ...
    if (!DBD::drizzle::st::_prepare($sth, $statement, $attribs)) {
	$sth = undef;
    }
...
sub db2ANSI {
    my $self = shift;
    my $type = shift;
    return $DBD::drizzle::db::db2ANSI{"$type"};
}

sub ANSI2db {
    my $self = shift;
    my $type = shift;
    return $DBD::drizzle::db::ANSI2db{"$type"};
}
...
sub get_info {
    my($dbh, $info_type) = @_;
    require DBD::drizzle::GetInfo;
    my $v = $DBD::drizzle::GetInfo::info{int($info_type)};
...
package DBD::drizzle::st; # ====== STATEMENT ======
use strict;
...
=head1 SYNOPSIS
    ...
    $dsn = "DBI:drizzle:database=$database;host=$hostname;port=$port";

    $dbh = DBI->connect($dsn, $user, $password);


    $drh = DBI->install_driver("drizzle");
    @databases = DBI->data_sources("drizzle");
       or
    @databases = DBI->data_sources("drizzle",
...
=head1 EXAMPLE
  ...
  my $dbh = DBI->connect("DBI:drizzle:database=test;host=localhost",
                         "joe", "joe's password",
                         {'RaiseError' => 1});
...
B<DBD::drizzle> is the Perl5 Database Interface driver for the MySQL
database. In other words: DBD::drizzle is an interface between the Perl
...
In what follows we first discuss the use of DBD::drizzle,
...
method like so:
  ...
  $dbh = DBI->connect("DBI:drizzle:database=$db;host=$host",
...
=item B<connect>
    ...
    $dsn = "DBI:drizzle:$database";
    $dsn = "DBI:drizzle:database=$database;host=$hostname";
    $dsn = "DBI:drizzle:database=$database;host=$hostname;port=$port";
...
=item drizzle_client_found_rows
...
Without drizzle_client_found_rows, if you perform a query like

  UPDATE $table SET id = 1 WHERE id = 1

then the MySQL engine will always return 0, because no rows have changed.
With drizzle_client_found_rows however, it will return the number of rows
that have an id 1, as some people are expecting. (At least for compatibility
to other engines.)

=item drizzle_compression

As of MySQL 3.22.3, a new feature is supported: If your DSN contains
the option "drizzle_compression=1", then the communication between client
and server will be compressed.

=item drizzle_connect_timeout

If your DSN contains the option "drizzle_connect_timeout=##", the connect
request to the server will timeout if it has not been successful after
the given number of seconds.

=item drizzle_read_default_file

=item drizzle_read_default_group
...
files unlike the client programs (drizzle, drizzleadmin, ...) that do, but
outside of the C client library. Thus you need to explicitly request
reading a config file, as in

    $dsn = "DBI:drizzle:test;drizzle_read_default_file=/home/joe/my.cnf";
    $dbh = DBI->connect($dsn, $user, $password)

The option drizzle_read_default_group can be used to specify the default
...
I<localhost>. However, by using
    ...
    $dsn = "DBI:drizzle:test;drizzle_read_default_group=perl;"
        . "drizzle_read_default_file=/home/joe/my.cnf";
    $dbh = DBI->connect($dsn, $user, $password);
...
the C function drizzle_options() for details.
...
If you turn drizzle_ssl on, you might also wish to use the following
flags:

=item drizzle_ssl_client_key

=item drizzle_ssl_client_cert

=item drizzle_ssl_ca_file

=item drizzle_ssl_ca_path

=item drizzle_ssl_cipher

These are used to specify the respective parameters of a call
to drizzle_ssl_set, if drizzle_ssl is turned on.  


=item drizzle_local_infile
...
"drizzle_local_infile=1", LOAD DATA LOCAL will be enabled.  (However,
...
=item B<ListDBs>
    ...
    my $drh = DBI->install_driver("drizzle");
...
=item drizzle_enable_utf8

This attribute determines whether DBD::drizzle should assume strings
...
10 of the drizzle manual for details.
...
The statement handles of DBD::drizzle support a number
...
is the C<drizzle_use_result> attribute: This forces the driver to use
drizzle_use_result rather than drizzle_store_result. The former is faster
and less memory consuming, but tends to block other processes. (That's why
drizzle_store_result is the default.)

To set the C<drizzle_use_result> attribute, use either of the following:

  my $sth = $dbh->prepare("QUERY", { "drizzle_use_result" => 1});

or

  my $sth = $dbh->prepare("QUERY");
  $sth->{"drizzle_use_result"} = 1;
...
to DBD::drizzle. The attribute list includes:
...
way for accessing this attribute is via $dbh->{'drizzle_insertid'}.
(Note we are using the $dbh in this case!)

=item drizzle_is_blob
...
=item drizzle_is_pri_key

Reference to an array of boolean values; TRUE indicates, that the
respective column is a primary key.

=item drizzle_is_auto_increment
...
a meaningfull equivalent, for example DBD::drizzle::FIELD_TYPE_INTERVAL
is mapped to DBI::SQL_VARCHAR().
If you need the native column types, use I<drizzle_type>. See below.

=item drizzle_type

A reference to an array of MySQL's native column types, for example
DBD::drizzle::FIELD_TYPE_SHORT() or DBD::drizzle::FIELD_TYPE_STRING().
Use the I<TYPE> attribute, if you want portable types like
DBI::SQL_SMALLINT() or DBI::SQL_VARCHAR().

=item drizzle_type_name

Similar to drizzle, but type names and not numbers are returned.
Whenever possible, the ANSI SQL name is preferred.

=item drizzle_warning_count
...
Beginning with DBD::drizzle 2.0416, transactions are supported.
...
DBD::drizzle has a "reconnect" feature that handles the so-called
...
The "reconnect" feature of DBD::drizzle can be toggled by using the
L<drizzle_auto_reconnect> attribute. This behaviour should be turned off
in code that uses LOCK TABLE because if the database server time out
and DBD::drizzle reconnect, table locks will be lost without any 
...
As of version 3.0002_5, DBD::drizzle supports multiple result sets (Thanks
...
DBD::drizzle code snippits will be added in the future.
...
The multithreading capabilities of DBD::drizzle depend completely
...
thread unsafe functions are called. Thus DBD::drizzle is believed
...
DBD::drizzle. However, you need at least the client
libraries and possibly the header files, if you are compiling DBD::drizzle
...
Then enter the following commands (note - versions are just examples):
  ...
  cd ..
  gzip -cd DBD-drizzle-(version)-tar.gz | tar xf -
  cd DBD-drizzle-(version)
  perl Makefile.PL
  make
  make test
  make install
...
For example, of your file F<drizzle.h> is in F</usr/include/drizzle/drizzle.h>,
then enter the header directory F</usr>, likewise for
F</usr/lib/drizzle/libdrizzleclient.a> or F</usr/lib/libdrizzleclient.so>.
...
As of this writing, DBD::drizzle is missing in the ActivePerl 5.8.0
repository. However, Randy Kobes has kindly donated an own
distribution and the following might succeed:

  ppm install http://theoryx5.uwinnipeg.ca/ppms/DBD-drizzle.ppd
...
I recommend using the win32clients package for installing DBD::drizzle
...
Extract sources into F<C:\>. This will create a directory F<C:\drizzle>
...
directory. If the server is already installed in F<C:\drizzle>,
...
installed DBD::drizzle.

=item -

Extract the DBD::drizzle sources into another directory, for
...
enter a 1 here (MySQL only), and
  ...
  Where is your drizzle installed? Please tell me the directory that
  contains the subdir include.

where you have to enter the win32clients directory, for example
F<C:\drizzle> or F<C:\tmp\drizzle>.
...
modify the above steps as follows: Run
  ...
  perl Makefile.PL NAME=DBD-drizzle BINARY_LOCATION=DBD-drizzle.tar.gz
  nmake ppd
  nmake
...
distribution) to create an archive:
  ...
  mkdir x86
  tar cf x86/DBD-drizzle.tar blib
  gzip x86/DBD-drizzle.tar

Put the files x86/DBD-drizzle.tar.gz and DBD-drizzle.ppd onto some WWW server
and install them by typing

  install http://your.server.name/your/directory/DBD-drizzle.ppd
...
This module is maintained and supported on a mailing list,
    ...
    perl@lists.drizzle.com

To subscribe to this list, go to

http://lists.drizzle.com/perl?sub=1

Mailing list archives are available at

http://lists.drizzle.com/perl
...
such as DBD::drizzle version, DRIZZLE version, OS type/version, etc
to this link:
